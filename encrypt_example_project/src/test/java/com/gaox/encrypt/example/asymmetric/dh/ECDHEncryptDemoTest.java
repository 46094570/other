...
public class ECDHEncryptDemoTest {
    //甲方公钥
    ...
    public final void initKey() throws Exception {
        //生成甲方密钥对
        Map<String, Object> keyMap1 = ECDHEncryptDemo.initKey();
        publicKey1 = ECDHEncryptDemo.getPublicKey(keyMap1);
        privateKey1 = ECDHEncryptDemo.getPrivateKey(keyMap1);
        System.out.println("甲方公钥:\n" + Base64.encodeBase64String(publicKey1));
        System.out.println("甲方私钥:\n" + Base64.encodeBase64String(privateKey1));

        //根据甲方公钥生成乙方密钥对
        Map<String, Object> keyMap2 = ECDHEncryptDemo.initKey(publicKey1);
        publicKey2 = ECDHEncryptDemo.getPublicKey(keyMap2);
        privateKey2 = ECDHEncryptDemo.getPrivateKey(keyMap2);
        System.out.println("乙方公钥:\n" + Base64.encodeBase64String(publicKey2));
        System.out.println("乙方私钥:\n" + Base64.encodeBase64String(privateKey2));

        //根据甲方私钥乙方公钥生成甲方本地密钥
        key1 = ECDHEncryptDemo.getSecretKey(publicKey2, privateKey1);
        System.out.println("甲方本地密钥:\n" + Base64.encodeBase64String(key1));
        //根据甲方公钥乙方私钥生成乙方本地密钥
        key2 = ECDHEncryptDemo.getSecretKey(publicKey1, privateKey2);
...
//        assertEquals(key1, key2);
    ...
    public final void test() throws Exception {
        System.out.println("\n========甲方向乙方发送加密数据=======");
        String input1 = "ECDH密码交换算法";
        System.out.println("甲方原文:\t" + input1);
        System.out.println("----使用甲方本地密钥对数据加密----");
        byte[] code1 = ECDHEncryptDemo.encrypt(input1.getBytes(), key1);
        System.out.println("加密:\t" + Base64.encodeBase64String(code1));
        System.out.println("----乙方接收到加密数据，使用乙方本地密钥对数据解密----");
        byte[] decode1 = ECDHEncryptDemo.decrypt(code1, key2);
...
//        assertEquals(input1, decode1);
        ...
        System.out.println("乙方原文:\t" + input2);
        System.out.println("----使用乙方本地密钥对数据加密----");
        byte[] code2 = ECDHEncryptDemo.encrypt(input2.getBytes(), key2);
        System.out.println("加密:\t" + Base64.encodeBase64String(code2));
        System.out.println("----甲方接收到加密数据，使用甲方本地密钥对数据解密----");
        byte[] decode2 = ECDHEncryptDemo.decrypt(code2, key1);
        String output2 = new String(decode2);
        System.out.println("甲方解密数据:\t" + output2);
